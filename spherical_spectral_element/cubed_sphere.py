...
def edge_match(free_idx, id_edge_in, id_edge_out, is_forwards):
...
elem_id_fn = lambda face_id, x_id, y_id: face_id * nx**2 + x_id * nx + y_id
def inv_elem_id_fn(idx):
    ...
    for face_idx in [TOP_FACE, BOTTOM_FACE, FRONT_FACE, BACK_FACE, LEFT_FACE, RIGHT_FACE]:
      for x_idx in range(nx):
        for y_idx in range(nx):
          f, x, y = inv_elem_id_fn(elem_id_fn(face_idx, x_idx, y_idx))
  ...
  for face_idx in [TOP_FACE, BOTTOM_FACE, FRONT_FACE, BACK_FACE, LEFT_FACE, RIGHT_FACE]:
    for x_idx in range(nx):
      for y_idx in range(nx):
        ...
        top_info = [elem_id_fn(face_idx, x_idx, y_idx-1), BOTTOM_EDGE, FORWARDS]
        left_info = [elem_id_fn(face_idx, x_idx-1, y_idx), RIGHT_EDGE, FORWARDS]
        right_info = [elem_id_fn(face_idx, x_idx+1, y_idx), LEFT_EDGE, FORWARDS]
        bottom_info = [elem_id_fn(face_idx, x_idx, y_idx+1), TOP_EDGE, FORWARDS]
        if x_idx == 0:
          edge_idx = LEFT_EDGE
          free_idx = y_idx
          face_pair, edge_pair, edge_dir = face_topo[face_idx][edge_idx]
          x_idx_out, y_idx_out = edge_match(free_idx, edge_idx, edge_pair, edge_dir)
          left_info = [elem_id_fn(face_pair, x_idx_out, y_idx_out), edge_pair, edge_dir]
        if x_idx == nx-1:
          edge_idx = RIGHT_EDGE
          free_idx = y_idx
          face_pair, edge_pair, edge_dir = face_topo[face_idx][edge_idx]
          x_idx_out, y_idx_out = edge_match(free_idx, edge_idx, edge_pair, edge_dir)
          right_info = [elem_id_fn(face_pair, x_idx_out, y_idx_out), edge_pair, edge_dir]
        if y_idx == nx-1:
          edge_idx = BOTTOM_EDGE
          free_idx = x_idx
          face_pair, edge_pair, edge_dir = face_topo[face_idx][edge_idx]
          x_idx_out, y_idx_out = edge_match(free_idx, edge_idx, edge_pair, edge_dir)
          bottom_info = [elem_id_fn(face_pair, x_idx_out, y_idx_out), edge_pair, edge_dir]
        if y_idx == 0:
          edge_idx = TOP_EDGE
          free_idx = x_idx
          face_pair, edge_pair, edge_dir = face_topo[face_idx][edge_idx]
          x_idx_out, y_idx_out = edge_match(free_idx, edge_idx, edge_pair, edge_dir)
          top_info = [elem_id_fn(face_pair, x_idx_out, y_idx_out), edge_pair, edge_dir]
      ...
      for x_idx in range(nx):
        for y_idx in range(nx):
          for edge_idx in [TOP_EDGE, LEFT_EDGE, RIGHT_EDGE, BOTTOM_EDGE]:
            ...
            face_idx_pair, x_idx_pair, y_idx_pair  = inv_elem_id_fn(idx_pair)
...
face_connectivity, face_position, face_position_2d = gen_cube_topo()
