...
vert_redundancy = gen_vert_redundancy(face_connectivity, face_position)



p_points = np.array([1.0, np.sqrt(1/5), -np.sqrt(1/5), -1.0])
g_weights = np.array([1/6, 5/6, 5/6, 1/6])
assert(np.abs((np.sum(g_weights * p_points**2) - 2/3 ) < 1e-10))
npt = len(p_points)


def generate_derivative():
  # uses the "standard interpolating polynomials" or whatever
  ...
  if TESTING:
    assert(np.max(np.abs(np.dot(deriv, p_points**2 - p_points**3) -  (2*p_points - 3 * p_points**2)) < 1e-10))
